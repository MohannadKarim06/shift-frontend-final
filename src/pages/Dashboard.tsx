...
export const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    // Fetch featured workflow
    const fetchFeatured = async () => {
      try {
        const wq = query(collection(db, 'workflows'), limit(1));
        const snapshot = await getDocs(wq);
        if (!snapshot.empty) {
          setFeaturedWorkflow({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as Workflow);
        }
      } catch (error) {
        try {
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-zinc-900">{i === 1 ? 'John Doe' : i === 2 ? 'Ahmed S.' : 'Maria K.'}</p>
                      <p className="text-xs text-zinc-500">{t('creative')}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-zinc-900">{1200 - (i * 200)}</p>
